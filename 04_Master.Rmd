...
>*Answer 1:* Because the satellite collects data only from above, it can only view the fynbos on a 2D plane with a finite area. It cannot account for the vertical growth of the fynbos that would be increasing more linearly.
...
# fit model
fit_negexp <- nls(NDVI ~ alpha + gamma * (1 - exp(- age/lambda)),                data = dat, start = par, trace = F, 
...
>*Answer 2:* The full model slightly decreases the proposed effect of alpha and also increases the proposed effect of gamma. For both of these the standard error and evidence against the null hypothesis remain relatively constant. In the full model, the proposed effect of lambda is more noticeably decreased. With that its standard error is more greatly decreased and its evidence against the null hypothesis is greatly increased.
...
>*Answer 3:* The simple model has a residual sum of squares of 0.72 whereas the full model has a residual sum of squares of 0.45. This means that the full model is better because it does a far better job of describing the variance in the data set.

Q4: How many degrees of freedom are there in this ANOVA and why (i.e. what are they)?

>*Answer 4:* The simple model has 135 degrees of freedom, because it has 138 observations minus 3 variables whereas the full model has 133 degrees of freedom, because it has 138 observations minus 5 variables. The model has 2 degrees of freedom because the full model has two more variables.
...
>*Answer 5:* Yes. The full model has a far lower AIC of 451 155.3 compared to that of the simple model's 718 113.1. The difference in AIC is 266 957.8 which is quite large which indicates that the difference is significant enough for the full model to be preferred. 
...
>*Answer 6:* The results are formed differently and thus exist in two different and incomparable ranges.
